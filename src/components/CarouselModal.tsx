...
export default function CarouselModal({
  ...
  return (
          ...
          <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={onDragEnd}>
            <SortableContext items={orderedIds} strategy={rectSortingStrategy}>
              <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
                {members.map((a, i) => (
                  <SortableTile key={a.id} asset={a} index={i} onRemove={() => removeAsset(a.id)} onView={() => setView(i)} />
                ))}
              </div>
            </SortableContext>
          </DndContext>
    ...
    {view !== null && <Lightbox assets={members} startIndex={view} onClose={() => setView(null)} />}
    </>
  )
}

function SortableTile({ asset, index, onRemove, onView }: { asset: ContentAsset; index: number; onRemove: () => void; onView: () => void }) {
  ...
  return (
    ...
    >
      ...
      <button
        onClick={onView}
        className="absolute left-2 bottom-11 z-10 flex h-6 w-6 items-center justify-center rounded-full bg-black/60 text-xs text-white opacity-0 shadow transition-opacity group-hover:opacity-100"
        title="Open (then arrow through the carousel)"
      >
        ⤢
      </button>
      <button
